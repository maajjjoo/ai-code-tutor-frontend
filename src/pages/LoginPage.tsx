    ...
    <div className="min-h-screen bg-[#0d0d14] flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10">
        ...
        <div className="bg-[#161622] border border-[#ffffff0f] rounded-2xl p-6 shadow-2xl shadow-black/40">
          <h2 className="text-base font-semibold text-white mb-5">{UI.LOGIN}</h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label htmlFor="login-email" className="text-xs font-medium text-[#9ca3af]">{UI.EMAIL}</label>
              <input
                id="login-email"
                name="email"
                type="email"
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="login-password" className="text-xs font-medium text-[#9ca3af]">{UI.PASSWORD}</label>
              <input
                id="login-password"
                name="password"
                type="password"
              ...
              {form.password.length > 0 && form.password.length < 8 && (
                <p className="text-xs text-red-400 mt-[2px]">La contraseña debe tener al menos 8 caracteres</p>
              )}
            </div>
            <button type="submit" disabled={loading || !canSubmit}
              ...
              {loading ? 'Iniciando sesión...' : UI.LOGIN_BUTTON}
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-xs text-[#6b7280] mt-5">
          {UI.NO_ACCOUNT}{' '}
            <Link to="/register" className="text-[#60a5fa] hover:text-white transition-colors">{UI.REGISTER}</Link>
        </p>
      </div>
    </div>
  );
