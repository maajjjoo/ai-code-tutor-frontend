    ...
    <div className="h-screen w-screen grid overflow-hidden bg-white dark:bg-gray-900 transition-colors" style={{ gridTemplateColumns: `${p.sidebarWidth}px 1fr ${p.aiPanelWidth}px` }}>
      ...
      <div className="bg-white dark:bg-gray-900 border-r border-[#E5E7EB] dark:border-gray-700 flex flex-col overflow-hidden p-3 relative">
        ...
        <div className="flex-1 overflow-y-auto mt-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-[#9CA3AF] dark:text-gray-500 mb-[8px]">{UI.CURRENT}</p>
          {p.activeProject && (
          ...
          {!p.activeProject && (
            <div className="text-center py-6">
              ...
              <p className="text-xs text-[#9CA3AF] dark:text-gray-500">{UI.CREATE_PROJECT_START}</p>
              <button onClick={() => p.setIsNewProjectModalOpen(true)} className="mt-3 px-3 py-1.5 bg-[#534AB7] text-white text-xs rounded-lg hover:opacity-90 cursor-pointer">{UI.NEW_PROJECT_BUTTON}</button>
            </div>
          )}
                ...
                >
                  ...
                  {isRenaming ? (
                    ...
                    <>
                      ...
                      <button onClick={e => { e.stopPropagation(); p.setFsNodes((prev: VNode[]) => prev.filter((n: VNode) => n.id !== f.id)); if (p.fsActiveId === f.id) { p.setFsActiveId(null); p.setOpenFile(null); p.setCode(''); } delete p.fileContentsRef.current[f.id]; showToast(UI.FILE_DELETED, 'success'); }}
          ...
          {p.activeProject && (
            ...
            >
              ...
              <span className="text-[12px] text-[#9CA3AF] dark:text-gray-500">{UI.NEW_FILE}</span>
            </div>
          )}
        </div>
        ...
        <div className="overflow-y-auto max-h-[180px]">
          <p className="text-[11px] font-medium uppercase tracking-wider text-[#9CA3AF] dark:text-gray-500 mb-2">{UI.SAVED_PROJECTS}</p>
          {p.savedProjects.length === 0 && (
            <div className="flex flex-col items-center gap-1 py-4">
              ...
              <p className="text-[11px] text-[#9CA3AF] dark:text-gray-500">{UI.NO_SAVED_PROJECTS}</p>
              <p className="text-[10px] text-[#D1D5DB] dark:text-gray-600">{UI.CREATE_PROJECT_START}</p>
            </div>
          )}
        ...
        <div className="flex-1 flex overflow-hidden relative">
          {p.loadingProject && (
            <div className="absolute inset-0 z-10 bg-white/80 dark:bg-gray-900/80 flex flex-col items-center justify-center">
              ...
              <p className="text-[13px] text-[#4B5563] dark:text-gray-400">{UI.LOADING_PROJECT}</p>
            </div>
          )}
          {!p.openFile ? (
            <div className="flex-1 flex items-center justify-center bg-white dark:bg-gray-900 select-none">
              <div className="text-center">
                ...
                <p className="text-sm text-[#9CA3AF] dark:text-gray-500">{UI.OPEN_FILE}</p>
                <p className="text-xs mt-2 text-[#C4C4C4]">{UI.SAVE_HINT}</p>
              </div>
            </div>
          ) : (
      ...
      {p.isCreatingProject && (
        <div className="absolute inset-0 bg-white/80 dark:bg-gray-900/80 flex flex-col items-center justify-center z-50 gap-3">
          ...
          <p className="text-[#534AB7] text-sm font-medium">{UI.PREPARING_EXERCISE}</p>
        </div>
      )}
      ...
      <div className="md:hidden fixed inset-0 bg-white dark:bg-gray-900 z-[100] flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-[15px] font-medium text-[#111827] dark:text-gray-100">Escritorio requerido</p>
          <p className="text-[13px] text-[#4B5563] dark:text-gray-400 mt-2">Usa un navegador de escritorio para el editor.</p>
        </div>
      </div>
    </div>
  );
