...
export function CompletionModal({ courseName, level, isOpen, isLastLevel, onClose, onPractice, onNextLevel }: Props) {
  ...
  const levelUI = UI[level.toUpperCase() as keyof typeof UI];
  const nextLevelUI = level === 'beginner' ? UI.INTERMEDIATE : UI.ADVANCED;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        ...
        aria-modal="true"
      >
         <button onClick={onClose} title={UI.CLOSE} className="absolute top-4 right-4 text-[#9CA3AF] dark:text-gray-500 hover:text-[#111827] dark:hover:text-gray-100 cursor-pointer transition-colors">
        ...
        <div className="w-12 h-12 bg-[#E1F5EE] dark:bg-teal-900/30 rounded-full flex items-center justify-center mx-auto mb-3.5 transition-colors">
         ...
         <h2 className="text-[15px] font-medium text-[#111827] dark:text-gray-100">{levelUI} {UI.LEVEL_COMPLETE}!</h2>
         <p className="text-[13px] text-[#4B5563] dark:text-gray-400 mt-1.5">{UI.YOU_COMPLETED} {levelUI} {UI.OF} {courseName}</p>
        ...
        <div className="flex flex-col gap-2">
          <button
           ...
           >
             {UI.PRACTICE_WHAT_YOU_LEARNED}
           </button>
           {isLastLevel ? (
             <p className="text-[13px] text-[#4B5563] font-medium py-2">
               {UI.YOU_MASTERED} {courseName}!
             </p>
           ) : (
             <button
