...
export function InlineAiPanel({ currentCode, language, exerciseStatement }: Props) {
  ...
  return (
    <div
      ...
      style={{ height: panelHeight, transition: 'height 200ms ease-in-out' }}
    >
      {/* Panel header */}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 flex flex-col gap-2">
        {messages.length === 0 && (
          <p className="text-[11px] text-[#45475a] text-center py-2">
            Pregúntame sobre el ejercicio o pide una pista.
          </p>
        )}

        {/* Collapsed: show last 2 messages. Expanded: show all */}
        {(isExpanded ? messages : lastTwoMessages).map(message => (
          <div key={message.id} className={`flex gap-2 ${message.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-[9px] font-bold
              ${message.role === 'ai' ? 'bg-[#89b4fa]/20 text-[#89b4fa]' : 'bg-[#313244] text-[#6c7086]'}`}>
              {message.role === 'ai' ? 'AI' : 'Tú'}
            </div>
            <div
              className={`max-w-[85%] rounded-lg px-2.5 py-1.5 text-[11px] leading-relaxed
                ${message.role === 'user'
                  ? 'bg-[#1e1e2e] text-[#6c7086]'
                  : 'bg-[#0d0d14] border border-[#1e1e2e] text-[#a6adc8]'
                }`}
          ...
          <div className="flex gap-2">
            ...
            <div className="bg-[#0d0d14] border border-[#1e1e2e] rounded-lg px-2.5 py-1.5 flex items-center gap-1">
              {[0, 1, 2].map(index => (
                <span
                  key={index}
                  className="w-1 h-1 bg-[#585b70] rounded-full"
                  style={{ animation: `bounce 1s ease-in-out ${index * 0.15}s infinite` }}
                />
              ))}
            </div>
          </div>
