...
export function EditorConsole({ consoleTab, termLines, onTabChange, onClear }: Props) {
  return (
    <div className="h-[150px] bg-[#FAFAFA] dark:bg-gray-800 border-t border-[#E5E7EB] dark:border-gray-700 flex flex-col shrink-0 transition-colors">
      <div className="h-[32px] bg-[#F3F4F6] dark:bg-gray-700 border-b border-[#E5E7EB] dark:border-gray-700 flex items-center px-[14px] gap-4 shrink-0">
        <button onClick={() => onTabChange('Terminal')} className={`text-[12px] cursor-pointer ${consoleTab === 'Terminal' ? 'text-[#111827] dark:text-gray-100 font-semibold' : 'text-[#9CA3AF]'}`}>{UI.TERMINAL}</button>
        <button onClick={() => onTabChange('Output')} className={`text-[12px] cursor-pointer ${consoleTab === 'Output' ? 'text-[#111827] dark:text-gray-100 font-semibold' : 'text-[#9CA3AF]'}`}>{UI.OUTPUT}</button>
        <button onClick={() => onTabChange('Problems')} className={`text-[12px] cursor-pointer ${consoleTab === 'Problems' ? 'text-[#111827] dark:text-gray-100 font-semibold' : 'text-[#9CA3AF]'}`}>{UI.PROBLEMS}</button>
      ...
      <div className="flex-1 overflow-y-auto px-[14px] py-[10px] font-mono text-[12px] leading-relaxed">
        {termLines.length === 0 && consoleTab !== 'Problems' && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-2">
            ...
            <span className="text-[#9CA3AF] text-[12px]">{UI.RUN_TO_SEE}</span>
          </div>
        )}
        {consoleTab === 'Problems' && termLines.length === 0 && (
          <div className="flex items-center justify-center h-full gap-2">
            ...
            <span className="text-[#9CA3AF]">{UI.NO_PROBLEMS}</span>
          </div>
        )}
